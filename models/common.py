...
class Concat_ACA(nn.Module):
    def __init__(self, channel, dimension=1):
        super(Concat_ACA,self).__init__()
        self.d = dimension
        self.att = AdaptiveCoordAtt(channel)
...
class DBH(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(DBH, self).__init__()
        self.conv = DCNv2(c1, c2, k, s, padding=1)
        self.bn = nn.BatchNorm2d(c2)
...
class GBH(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(GBH, self).__init__()
        self.conv = GhostConv(c1, c2, k, s, g)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.Hardswish() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
...
class DCNv2(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=1, g=1, act=True, dilation=1, deformable_groups=1):
...
class TransformerLayer(nn.Module):
    def __init__(self, c, num_heads):
        ...
        self.positional_encoding = PositionEncoding(c)
...
class TRencoder(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        c_ = int(c2 * e)
        self.m = TransformerBlock(c_, c_, 2, n)
    def forward(self,x):
        return x
...
class fuseSPPCSPC(nn.Module):
    # CSP https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=5):
        super(fuseSPPCSPC, self).__init__()
...
class GSPPCSPC(nn.Module):
    # CSP https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=(5, 9, 13)):
        super(GSPPCSPC, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
...
class RepConv(nn.Module):
    ...
    ''' 重参数卷积
    训练时：不执行Model类的fuse函数,   一个3*3卷积、一个1*1卷积和一个BN层（当输入输出通道相同时）相加得到输出;
    推理时：在attempt_load函数加载训练好的模型时，会执行Model类的fuse函数，进而调用fuse_repvgg_block函数，实现将三个卷积重参数化，合并为一个3*3卷积输出
        训练时：
            deploy = False
...
class DiverseBranchBlock(nn.Module):
    def __init__(self, in_channels, out_channels, k,
                 s=1, p=None, g=1, act=nn.LeakyReLU(),
                 internal_channels_1x1_3x3=None,
